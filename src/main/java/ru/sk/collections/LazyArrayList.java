...
public class LazyArrayList<E> 
	...
	private final Callback<Integer, ? extends E> factory;
	...
	public LazyArrayList(Collection<? extends E> collection, int initialSize, Callback<Integer, ? extends E> factory, Comparator<E> comparator) {
		...
		if (collection == null) {
			addAll(Collections.<E>nCopies(initialSize, null));
		} else {
			if (collection.size() < initialSize) {

				addAll(collection);
				addAll(Collections.<E>nCopies(initialSize - collection.size(), null));
			} else {
				addAll(collection);
			}
		}
		
		this.rightShifts = new int[initialSize];
		this.leftShifts = new int[initialSize];
		this.factory = factory;
		this.comparator = comparator;
	}

	public LazyArrayList(int initialSize, Callback<Integer, ? extends E> factory, Comparator<E> comparator) {
		this(null, initialSize, factory, comparator);
	}

	public Callback<Integer, ? extends E> getFactory() {
		return factory;
	}
	...
	public int indexOf(Object o) {
		...
		if (o == null) {
			...
			for (int i = 0; i < size; i++) {
				if (comparator.compare((E) o, elementData(i)) == 0) {
					return i;
				}
			}
		}
	...
	public Object clone() {
		try {
			LazyArrayList<?> v = (LazyArrayList<?>) super.clone();
	...
	public E set(int index, E element) {
		...
		if (element == null) {
			throw new UnsupportedOperationException("Setting of null elements is not supported");
		}
	...
	public boolean add(E e) {
		...
		if (e == null) {
			throw new UnsupportedOperationException("Addition of null elements is not supported");
		}
	...
	public boolean remove(Object o) {
		...
		if (o != null) {
			for (int index = 0; index < size; index++) {
				if (comparator.compare((E) o, elementData(index)) == 0) {
	...
	private void updateRightShifts(int index) {
		...
		int[] newRightShifts = Arrays.copyOf(rightShifts, rightShifts.length - 1);
		
		for (int i = index + 1; i < rightShifts.length; i++) {
			newRightShifts[i - 1] = rightShifts[i] + 1;
		}
	...
	private class Itr 
		...
		public E next() {
			...
			Object[] elementData = LazyArrayList.this.elementData;
			...
			return LazyArrayList.this.elementData(lastRet = i);
		}
		...
		public void forEachRemaining(Consumer<? super E> consumer) {
			...
			final int size = LazyArrayList.this.size;
			...
			final Object[] elementData = LazyArrayList.this.elementData;
			...
			while (i != size && modCount == expectedModCount) {
				consumer.accept(LazyArrayList.this.elementData(i++));
			}
	...
	class SubList 
		extends LazyArrayList<E> 
		implements RandomAccess {

		/**
		 * 
		 */
		private static final long serialVersionUID = -8805079463795248346L;

		private final LazyArrayList<E> parent;
		private final int parentOffset;
		private final int offset;
		int size;

		SubList(LazyArrayList<E> parent, int offset, int fromIndex, int toIndex) {

			super(toIndex - fromIndex, factory, comparator);

			this.parent = parent;
			this.parentOffset = fromIndex;
			this.offset = offset + fromIndex;
			this.size = toIndex - fromIndex;
			this.modCount = LazyArrayList.this.modCount;
		}
		...
		public E set(int index, E e) {
			...
			E oldValue = LazyArrayList.this.elementData(offset + index);
			
			LazyArrayList.this.elementData[offset + index] = e;
		...
		public E get(int index) {
			...
			return LazyArrayList.this.elementData(offset + index);
		}
			...
			return new ListIterator<E>() {
				...
				public void remove() {
					...
					try {
						...
						expectedModCount = LazyArrayList.this.modCount;
					} catch (IndexOutOfBoundsException ex) {
						throw new ConcurrentModificationException();
					}
				}
				...
				public void set(E e) {
					...
					try {
						LazyArrayList.this.set(offset + lastRet, e);
					} catch (IndexOutOfBoundsException ex) {
						throw new ConcurrentModificationException();
					}
				}
				...
				final void checkForComodification() {
					if (expectedModCount != LazyArrayList.this.modCount)
						throw new ConcurrentModificationException();
				}
			};
		}
		...
		private void checkForComodification() {
			if (LazyArrayList.this.modCount != this.modCount) {
				throw new ConcurrentModificationException();
			}
		}
		...
		public Spliterator<E> spliterator() {
			checkForComodification();
			return new ArrayListSpliterator<E>(LazyArrayList.this, offset, offset + this.size, this.modCount);
		}
	}
	...
	static final class ArrayListSpliterator<E> implements Spliterator<E> {
		...
		private final LazyArrayList<E> list;
		private int index; // current index, modified on advance/split
		private int fence; // -1 until used; then one past last index
		private int expectedModCount; // initialized when fence set

		/** Create new spliterator covering the given range */
		ArrayListSpliterator(LazyArrayList<E> list, int origin, int fence, int expectedModCount) {
